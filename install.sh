...
usage()
{
    ...
    echo ""
    echo "Example: $PROG /dev/vda https://github.com/thejaykobe/k3os/releases/download/v0.8.0/k3os.iso"
    echo ""
...
do_format()
{
    ...
    if [ "$PARTTABLE" = "gpt" ]; then
        ...
        parted -s ${DEVICE} mkpart primary ext4 50MB 2GB
    else
        BOOT_NUM=
        STATE_NUM=1
        parted -s ${DEVICE} mkpart primary ext4 0% 2GB
    fi
