...
menuentry "k3OS Current" {
  ...
  set root=(\$root)
  linux (loop0)/vmlinuz printk.devkmsg=on cgroup_no_v1=all systemd.unified_cgroup_hierarchy=1 console=tty1 $GRUB_DEBUG
...
menuentry "k3OS Rescue (current)" {
  ...
  loopback loop0 /\$sqfile
  set root=(\$root)
  linux (loop0)/vmlinuz printk.devkmsg=on cgroup_no_v1=all systemd.unified_cgroup_hierarchy=1 rescue console=tty1
  initrd /k3os/system/kernel/current/initrd
}
