...
Upgrades are organized into per-minor channels. Every image ships the channel plan for each maintained minor (`k3os-1.33.yaml`, `k3os-1.34.yaml`, `k3os-1.35.yaml`) under https://github.com/thejaykobe/k3os/tree/v1.35.x/overlay/share/rancher/k3s/server/manifests/system-upgrade-plans

To put a node on a channel, label it with that channel's name -> "kubectl label nodes --all 'k3os.io/upgrade=latest-1.35.x' --overwrite"

Because every image carries every channel, a node can be moved between channels (e.g. `latest-1.34.x` -> `latest-1.35.x`) just by changing that label. Each plan's pinned `version` is bumped per release by `scripts/set-channel-version` (run via the "Update Upgrade Channel" workflow before tagging), which fans the bump out across all release branches so nodes on a channel auto-upgrade as new patches land on that minor.
